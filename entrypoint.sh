...
curl "${TLGRM_URL}Telegram%20Alerting%has%20been%20started"
...
iterateArray() {
	...
	echo ">>>>Device Count: $deviceCount, smart params count: $smartCount"
		...
		echo "Cheking /dev/$device with smart param $param"
                smartResult=$(smartctl -A -v 1,raw48:54 /dev/$device | sed 's/^[[:space:]]*//' | sed 's/^0//' | sed -n -e "/^$param[[:space:]]/p" | awk '{ print $10 }')
		...
		if [ -z $smartResult  ]; then 
			...
			paramName=$(smartctl -A -v 1,raw48:54 /dev/$device | sed 's/^[[:space:]]*//' | sed 's/^0//' | sed -n -e "/^$param[[:space:]]/p" | awk '{ print $2 }')
